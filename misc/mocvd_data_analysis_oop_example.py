...
def liters_to_grams(liters, source):
    ...
    partial_pressure_torr = 10 ** (source.A - (source.B / (source.t + STANDARD_TEMPERATURE_KELVIN)))
    mols = (partial_pressure_torr * liters) / (R_TORR_LITERS * (source.t + STANDARD_TEMPERATURE_KELVIN))
    grams = source.molar_mass * mols
...
test = [0, 1, 1, 0, 0, 0, 1, 1]  # valve positions
check = lambda lst: lst[0] == 1 and lst[5] ==1
...
class TMAl_Source:
    def __init__(self):
        ...
        self.check_function = lambda lst: True

    def calculate_mass(self, row):
        '''
        A method to calculate mass given a row of data from the SMI logs.
        '''
        if self.check_function(row):
            # do some logic to calculate amount
            return 1
        return 0
    
    def __str__(self):
        return f"A {self.name} source"
...
for row in data:
    ...
    for source in source_lst:
        line.append(source.calculate_mass(row))
    output_lst.append(line)


for source in source_lst:
    df[source.name] = source.calculate_mass(df)
...
mass_functions = [calc_TMAl, calc_TMGa]
for mass_func in mass_functions:
    df[mass_func.__name__] = mass_func(df)
